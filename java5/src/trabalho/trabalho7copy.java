...
public class trabalho7copy {
    ...
    private static final Map<String, Integer> derrotas = new HashMap<>(); // Mapa de derrotas
    ...
    private static void salvarDerrota(String jogador) {
        PrintStream out = System.out;
        int derrotasJogador = derrotas.getOrDefault(jogador, 0) + 1;
        derrotas.put(jogador, derrotasJogador);
        out.println("Derrota registrada com sucesso para " + jogador + "!");
    }
    ...
    private static void exibirPontuacoes() {
        ...
        for (Map.Entry<String, Integer> entry : pontuacoes.entrySet()) {
            out.println("O jogador " + entry.getKey() + " acumulou um total de " + entry.getValue() + " vitória(s) e " + derrotas.getOrDefault(entry.getKey(), 0) + " derrota(s)."); // Exibe as vitórias e derrotas do jogador
        }
